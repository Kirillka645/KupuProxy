...
class LocalizationResourcesTest {
    ...
    private val projectDir: File = File(System.getProperty("user.dir")!!)
        .let { dir -> if (dir.name == "app") dir.parentFile else dir }
    ...
    fun allLocalesHaveDirectories() {
        ...
        AppLocaleManager.supportedLocales.forEach { locale ->
            ...
            Assert.assertEquals(
                "Key mismatch: ${locale.languageTag}",
                default.keys, catalog.keys
            )
        }
    }
    ...
    private fun resourceDirName(languageTag: String): String = when (languageTag) {
        "en" -> "values"
        "pt-BR" -> "values-pt-rBR"
        "id" -> "values-in"
        "zh-CN" -> "values-zh-rCN"
